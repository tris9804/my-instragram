...
class PostViewSet(viewsets.ModelViewSet, CanLikeMixin):
    ...
    def get_queryset(self):
        ...
        if self.action == 'list' :
            following_user_list = self.request.user.following.filter(is_agreed=True).values_list('to_user', flat=True)
            queryset = queryset.filter(creator_id__in=following_user_list,)
